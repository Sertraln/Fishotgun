...
# class Noise:
#     def __init__(self):
#         self.amp = uniform(0.5, 1.5)
#         self.freq = uniform(0.5, 2.0)
#         self.phase = uniform(0, 2*pi)
#         self.x = 0
#         self.step = 0.1

#     def next(self):
#         """Renvoie la valeur de la sinusoïde au point x+step."""
#         self.x += self.step
#         return self.amp * sin(self.freq * self.x + self.phase)
...
class PerlinNoise():
    def __init__(self, 
            seed=1, amplitude=1, frequency=1, 
    ...
    def get(self, x):
        ...
        for _ in range(self.octaves):
            result += self.interpolated_noise(x * frequency) * amplitude
            frequency *= 2
            amplitude /= 2
